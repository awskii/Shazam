//Author of this library is Matthew Kwan (1997).
//I've just modified enc/dec method and wrote some 
//wrappers to encrypt strings and binaries.
...
unsigned long keyschedule[16][3] = {
...
IceKey::IceKey (int n)
{
  ...
  for(int i = 0; i < _rounds; i++) {
    for(int j = 0; j < 3; j++) {
      _keysched[i].val[j] = keyschedule[i][j];
    }
  }
}
...
void IceKey::encBinary(const unsigned char *data, long size) {
  int len = (size /8 +1) * 8;
  unsigned char plaintext[len], ciphertext[len];
  for(int i = 0; i < size; i++) {
    plaintext[i] = data[i];
  }
  for(int i = 0; i < len; i+=8) {
    this->encrypt((const unsigned char *)plaintext,ciphertext, i);
  }
...
) const
{
  ...
  cout << (ptext[0]<<24 ) << endl;
  l = (((unsigned long) ptext[idx]) << 24)
        | (((unsigned long) ptext[idx + 1]) << 16)
        | (((unsigned long) ptext[idx + 2]) << 8) | ptext[idx + 3];
  r = (((unsigned long) ptext[idx + 4]) << 24)
        | (((unsigned long) ptext[idx + 5]) << 16)
        | (((unsigned long) ptext[idx + 6]) << 8) | ptext[idx + 7];
  ...
  for (i = 0; i < 4; i++) {
      ctext[idx + 3 - i] = r & 0xff;
      ctext[idx + 7 - i] = l & 0xff;
...
) const
{
  ...
  l = (((unsigned long) ctext[idx]) << 24)
        | (((unsigned long) ctext[idx + 1]) << 16)
        | (((unsigned long) ctext[idx + 2]) << 8) | ctext[idx + 3];
  r = (((unsigned long) ctext[idx + 4]) << 24)
        | (((unsigned long) ctext[idx + 5]) << 16)
        | (((unsigned long) ctext[idx + 6]) << 8) | ctext[idx + 7];
  ...
  for (i = 0; i < 4; i++) {
      ptext[idx+3 - i] = r & 0xff;
      ptext[idx+7 - i] = l & 0xff;
...
void IceKey::encString(const unsigned char *ptext) {
  int length = strlen((char*)ptext);
  int length2 = (length / 8 +1)*8;

  unsigned char plaintext[length2];
  unsigned char ciphertext[length2];
...
// void IceKey::decString(unsigned char *ctext) {
//   unsigned char ptext[8];
//   for(int i = 0; i < 8; i++) {
//     ptext[i] = 0;
//   }
//   for(int i = 0; i < )
//   this->decrypt((const unsigned char*)ctext, ptext, 0);
//   for(int i =0; i < 16; i++) {
//     printf("%d", ptext[i]);
//   }
// }
...
int main(int argc, char *argv[]) {
  const unsigned char usage[] = "ICE Encryptor for Shazam. Usage: \n\t-f\tencrypt file;\n\t-s\tencrypt string;";
  if (argc < 3) {
    cout << usage << endl;
    return 0;
  }

  IceKey key = IceKey::IceKey(1);
  const unsigned char *ptext;
  ...
  if(!strcmp(argv[1], "-f")) {
    ...
    ptext = (const unsigned char *)contents.c_str();
    key.encBinary(ptext, size);
  } else if (!strcmp(argv[1],"-s")) {
    ptext = (const unsigned char*)argv[2];
    key.encString(ptext);
    // } else if (!strcmp(argv[1], "-d")) {
    //   unsigned char *ctext;
    //   ctext = (unsigned char *)argv[2];
    //   key.decString(ctext);
  } else {
    cout << usage << endl;
  }
